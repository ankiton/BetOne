            ...
            ModalDrawerSheet {
                ...
                listOf("Главная", "Ветка 1", "Ветка 2", "Ветка 3", "История", "Банк", "Настройки").forEach { item ->
                    ...
                    NavigationDrawerItem(
                        label = {
                            Text(
                                if (branchId != null) "${branchNames[branchId] ?: item}${if (hasPending) " (активно)" else ""}" else item,
                                color = MaterialTheme.colorScheme.onSurface
                            )
                        },
                        ...
                        colors = NavigationDrawerItemDefaults.colors(
                            selectedContainerColor = Color(0xFFAED581), // Бледно-оливковый для выделенного пункта
                            unselectedContainerColor = MaterialTheme.colorScheme.surface,
                            selectedTextColor = Color.White, // Белый текст на бледном фоне
